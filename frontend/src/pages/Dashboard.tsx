...
export function DashboardPage() {
	...
	const usagePercentage = usage?.currentPeriod
		? (usage.currentPeriod.screenshotsUsed /
				usage.currentPeriod.screenshotsLimit) *
			100
		: 0;
	...
	return (
		<div className="space-y-8">
			<div className="space-y-1">
				<h1 className="instrument-serif-regular text-4xl md:text-5xl font-normal tracking-tight">
					Dashboard
				</h1>
				...
				<CardContent className="space-y-4">
					<div className="flex items-center justify-between text-base">
						<span className="text-foreground">
							{usage?.currentPeriod?.screenshotsUsed || 0} /{" "}
							{usage?.currentPeriod?.screenshotsLimit || 0} screenshots
						</span>
					...
					{usagePercentage > 80 && (
						<p className="text-base text-muted-foreground">
							You're running low on screenshots. Contact support if you need
							higher limits.
						</p>
					)}
				</CardContent>
				...
				<CardHeader className="pb-4">
					<div className="flex items-center justify-between">
						<div className="space-y-1">
							...
							<CardDescription className="text-base">
								Your latest captured screenshots
							</CardDescription>
						</div>
						<Button
							variant="outline"
						...
						<div className="space-y-3">
							{screenshots.screenshots.map((screenshot) => (
								<button
									key={screenshot.id}
									type="button"
									className="w-full text-left flex items-center justify-between border border-border/40 rounded-lg p-4 hover:bg-accent/50 cursor-pointer transition-colors"
